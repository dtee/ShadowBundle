...
(function($) {
   ...
   var renderError = function(params) {
        ...
        var errorElement = $container.find(this.options.selector_error);
    ...
    AjaxForm.prototype.success = function(data, status, xhr) {
        ...
        // List of html elements that hosts errors
        var $errorContainers = $();
        ...
        for ( var index in errorList) {
            ...
            container = (!container || container.length == 0) ? this.$form : container;
            $errorContainers.add(container);
    ...
    AjaxForm.prototype.startSession = function(event) {
        ...
        // Reset all errors
        this.$form.find(this.options.selector_error).html('').css('display', 'none');
    ...
    $.ajaxForm = function(options) {
        AjaxForm.options = $.extend(true, AjaxForm.options, options);
        return AjaxForm;
    };
})(jQuery);
