...
class BalancedTeamValidator extends ConstraintValidator {
    /**
    ...
    public function isValid($game, Constraint $constraint) {
        ...
        foreach ( $game->getPlayers () as $player ) {
            if (! isset ( $chars [$player->getCharacter ()] )) {
                $this->setMessage ( "Character '{$player->getCharacter()}' does not exists.", array () );
        ...
        if ($hunters != $shadows) {
            $this->setMessage ( $constraint->message, array ('{{ hunters }}' => $hunters, '{{ shadows }}' => $shadows ) );
